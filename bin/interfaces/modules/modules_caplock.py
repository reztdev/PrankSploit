...
from bin.modules.payloads.Caplock import Cap_lock_Payload
import bin.interfaces.MainConsole
from bin.interfaces.source.help_interfaces.modules.help_caplock import (
    showHelpMessage, showOptionsMessage
)

filename = ''
class Payloads_Caplock(object):
...
def Main_Interactive_Caplock():
    try:
        while True:
            cmd = raw_input("{ln}prsf{ln1} payloads({rd}caplock_error{rd1}) > ".format(

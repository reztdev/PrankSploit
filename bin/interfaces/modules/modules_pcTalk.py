...
from bin.modules.payloads.pc_talk import PC_Talking
import bin.interfaces.MainConsole
from bin.interfaces.source.help_interfaces.modules.help_talk import (
    showHelpMessage, showOptionsMessage
)

filename = ''
class Payloads_PC_Talkings(object):
    ...
    def main_payload(self):
        try:
            ...
            if self.current_setting == "help":
                ...
                if filename != "":
                    talk = PC_Talking(filename)
                    talk.writing_payload()
                else:
                    print(showOptionsMessage(filename))
...
def Main_Interactive_Talking():
    try:
        while True:
            cmd = raw_input("{ln}prsf{ln1} payloads({rd}pc_talking{rd1}) > ".format(
                ln=pycolor_style.L, ln1=pycolor_style.W, rd=pycolor_style.R, rd1=pycolor_style.W
            ))
            cd = Payloads_PC_Talkings(cmd)
            cd.main_payload()
            if cmd == "exit":

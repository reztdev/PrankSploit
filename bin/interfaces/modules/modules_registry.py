...
from bin.modules.payloads.delete_registry import Delete_Registry_File
from bin.interfaces.source.help_interfaces.modules.help_registry import (
    showHelpMessage, showOptionsMessage
)

filename = ''
class Payloads_Delete_Registry(object):
    ...
    def main_payload(self):
        try:
            ...
            if self.current_setting == "help":
                ...
                if filename != "":
                    reg = Delete_Registry_File(filename)
                    reg.writing_payload()
                else:
                    print(showOptionsMessage(filename))
...
def Main_Interactive_Registry():
    try:
        while True:
            cmd = raw_input("{ln}prsf{ln1} payloads({rd}delete_registry{rd1}) > ".format(
                ln=pycolor_style.L, ln1=pycolor_style.W, rd=pycolor_style.R, rd1=pycolor_style.W
            ))
            cd = Payloads_Delete_Registry(cmd)
            cd.main_payload()
            if cmd == "exit":
